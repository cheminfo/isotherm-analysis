...
test('fromIGA', () => {
  ...
  expect(header.scan).toBe(1);
  expect(header.sampleWeight).toBe(9.2627);
  expect(header.sampleWeightDry).toBe(8.9674);

  const dataBlock = parseIGADataBlock(lines.slice(73, 103));
  expect(dataBlock.pressure[0]).toBeCloseTo(-0.07, 5);
  expect(dataBlock.gasDensity[0]).toBe(0.0);

  const meas = parseOneIGA(lines.slice(1, 106));
  expect(meas.meta.sampleNumber).toBe('0328');
  expect(meas.data.wtPercent).toHaveLength(30);
  expect(meas.data.wtPercent[29]).toBe(1.447);
